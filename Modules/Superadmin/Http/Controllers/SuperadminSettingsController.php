...
class SuperadminSettingsController extends BaseController
{
    ...
    public function edit()
    {
        ...
        $default_values = [
            ...
            'MAIL_DRIVER' => $is_demo ? null : config('mail.driver'),
            'MAIL_HOST' => $is_demo ? null : config('mail.host'),
            'MAIL_PORT' => $is_demo ? null : config('mail.port'),
            'MAIL_USERNAME' => $is_demo ? null : config('mail.username'),
            'MAIL_PASSWORD' => $is_demo ? null : config('mail.password'),
            'MAIL_ENCRYPTION' => $is_demo ? null : config('mail.encryption'),
            'MAIL_FROM_ADDRESS' => $is_demo ? null : config('mail.from.address'),
            'MAIL_FROM_NAME' => $is_demo ? null : config('mail.from.name'),
            'STRIPE_PUB_KEY' => $is_demo ? null : config('ms.payment_gateway.stripe_pub_key'),
            'STRIPE_SECRET_KEY' => $is_demo ? null : config('ms.payment_gateway.stripe_pub_key'),
            'PAYPAL_MODE' => config('paypal.mode'),
            'PAYPAL_SANDBOX_API_USERNAME' => $is_demo ? null : config('paypal.sandbox.username'),
            'PAYPAL_SANDBOX_API_PASSWORD' => $is_demo ? null : config('paypal.sandbox.password'),
            'PAYPAL_SANDBOX_API_SECRET' => $is_demo ? null : config('paypal.sandbox.secret'),
            'PAYPAL_LIVE_API_USERNAME' => $is_demo ? null : config('paypal.live.username'),
            'PAYPAL_LIVE_API_PASSWORD' => $is_demo ? null : config('paypal.live.password'),
            'PAYPAL_LIVE_API_SECRET' => $is_demo ? null : config('paypal.live.secret'),
            'BACKUP_DISK' => config('backup.backup.destination.disks'),
            'DROPBOX_ACCESS_TOKEN' => $is_demo ? null : config('filesystems.disks.dropbox.authorizationToken'),
            'RAZORPAY_KEY_ID' => $is_demo ? null : config('ms.payment_gateway.razorpay_key_id'),
            'RAZORPAY_KEY_SECRET'  => $is_demo ? null : config('ms.payment_gateway.razorpay_key_secret'),

            'PESAPAL_CONSUMER_KEY'  => $is_demo ? null : config('pesapal.consumer_key'),
            'PESAPAL_CONSUMER_SECRET'  => $is_demo ? null : config('pesapal.consumer_secret'),
            'PESAPAL_LIVE'  => $is_demo ? null : config('pesapal.live'),
            'PUSHER_APP_ID' => $is_demo ? null : config('broadcasting.connections.pusher.app_id'),
            'PUSHER_APP_KEY' => $is_demo ? null : config('broadcasting.connections.pusher.key'),
            'PUSHER_APP_SECRET' => $is_demo ? null : config('broadcasting.connections.pusher.secret'),
            'PUSHER_APP_CLUSTER' => $is_demo ? null : config('broadcasting.connections.pusher.options.cluster'),
            'GOOGLE_MAP_API_KEY' => $is_demo ? null : config('ms.settings.google_map_api'),
            'ALLOW_REGISTRATION' => $is_demo ? null : config('constants.allow_registration'),
            'PAYSTACK_PUBLIC_KEY' => $is_demo ? null : config('ms.payment_gateway.paystack_key_id'),
            'PAYSTACK_SECRET_KEY' => $is_demo ? null : config('ms.payment_gateway.paystack_key_secret'),
            'FLUTTERWAVE_PUBLIC_KEY' => $is_demo ? null : config('ms.settings.flutterwave_public_key'),
            'FLUTTERWAVE_SECRET_KEY' => $is_demo ? null : config('ms.settings.flutterwave_secret_key'),
            'FLUTTERWAVE_ENCRYPTION_KEY' => $is_demo ? null : config('ms.settings.flutterwave_encryption_key'),
        ];
